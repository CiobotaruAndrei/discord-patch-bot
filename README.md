...
- `/youtube videos show` - (admin) posteaza manual videoclipurile din ultima luna fara sa modifice deduplicarea automata; necesita un canal de destinatie configurat (`/youtube notify channel` sau o ruta), altfel raspunde clar ca lipseste. Primul lot (maximum 5) se posteaza **imediat** si durabil, iar restul continua in fundal in loturi de cate 5 la 10 minute (la restart in acest interval, reia comanda pentru loturile ramase).
