...
- runtime-ul comenzilor este construit ca `CommandRuntimeDependencies`, impartit in `discord`, `mongo`, `sources` si `platform`; fiecare sectiune expune numai capabilitatile permise, iar forma plata exista doar ca adaptor temporar pentru handler-ele migrate incremental;
- handler-ele sunt enumerate prin descriptori declarativi (`id`, `domain`, `build`), cu verificare de duplicate, in locul unei liste manuale fara identitate de domeniu;
...
- pipeline-ul comun `notificationFeedLoader` gestioneaza fetch, snapshot validat si fallback pentru update-uri si reduceri, iar serviciile pastreaza regulile specifice domeniului;
- task-urile periodice simple folosesc `ScheduledTaskRunner`, care impiedica suprapunerea, respecta oprirea si retine rezultatul ultimei executii;
- modulele aplicatiei si scripturile folosesc ESM; nu mai exista boundary-uri CommonJS in codul sursa;
- operatiile administrative multi-document sunt protejate de jurnalul cu lease-uri `pending -> leased -> done`, iar outbox-ul persista acceptarea livrarii inainte de finalizarea istoricului si a deduplicarii.
