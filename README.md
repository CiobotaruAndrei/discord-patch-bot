...
- `/add price-alert`, `/remove price-alert` si `/price-alert list` - (admin) gestioneaza alerte de pret. Alertele pot fi pregatite inainte de `/start reduceri` si raman inactive pana cand modulul reduceri are canal activ; apoi alerta se trimite pe acel canal, se declanseaza o singura data cand pretul ajunge la/sub prag si se rearmeaza fie cand pretul urca peste prag, fie cand jocul lipseste din feed-ul de reduceri `PRICE_ALERT_REARM_ABSENT_CYCLES` cicluri la rand (oferta s-a terminat si jocul a iesit din feed); rearmarea pe absenta nu se face pe esec de fetch al surselor.
...
- `/future-release add`, `/future-release delete`, `/future-release start` si `/future-release stop` sunt admin runtime; `/future-release list` este public pe server si afiseaza lista de maxim 20 jocuri si canalul configurat.
...
- handler-ele sunt enumerate prin descriptori declarativi cu `id`, `domain`, `scope`, `access`, `help`, `autocomplete`, `priority` si `build`, iar dispatcherul este ordonat dupa prioritate;
...
Compozitia runtime foloseste factory-uri cu dependinte explicite. Instantele Redis si registrul surselor sunt create in bootstrap si injectate; modulele de infrastructura pot expune doar fatade compatibile construite local, fara importuri din `app/runtimeComposition.ts`.

Granitele tiparii — ce ramane intentionat mai lax: (1) adaptoarele legacy `attachX(target)` exista doar pentru scripturi si teste izolate; factory-urile de productie primesc contracte explicite, iar registrele compun prin valori returnate si spread imutabil; (2) `commandRegistry` si `sourceRegistry` au contracte inchise, sunt compuse fara `installers: unknown[]` si sunt inghetate dupa boot; (3) payload-urile dinamice de la granitele externe sunt tipate `unknown` si ingustate la consum; (4) `Record<string, unknown>` ramas in handler-e reprezinta date locale Discord/Mongo, nu contracte de dependinte.

Starea curenta:

- handler-ele pentru comenzi cunoscute sunt separate in `src/features/command-handlers/`;
- routing-ul interactiunilor foloseste o lista tipata `CommandHandler[]` plus `Map<command, handler[]>`; subcomenzile sunt verificate de manifest, astfel `/future-release list` ramane public, iar `add/delete/start/stop` trec prin pre-check-ul admin; comenzile snoozed trec apoi prin `commandSnoozeGuard` inainte de dispatcher; nu mai exista lantul de `attachX` care impacheteaza `handleInteraction` si nici un fisier `interactions.ts` separat;
...
Registrele de wiring compun explicit factory-urile si valideaza fail-fast exporturile obligatorii. `sourceRegistryFactory` primeste `SourceRuntimeDeps`, iar `sourceRegistry` este fatada compatibila peste instanta construita cu dependinte locale sau injectata de bootstrap.
