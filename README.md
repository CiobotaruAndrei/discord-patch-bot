...
`npm run check` ruleaza si `check:layers` (`scripts/check-layer-imports.ts`), care impune regulile de dependinte intre straturi pe importurile relative din `app`/`features`/`domain`/`infra`/`sources`/`shared`: fara `infra -> app`, fara `sources -> app`, `domain`/`shared` raman pure (fara dependinte spre straturile de sus), features acceseaza Mongo doar prin repositories/DI (importurile de **valori** din `infra/mongo` pica; importurile de **tip** raman permise, sunt contracte) si fara **cicluri** de importuri runtime (muchiile type-only nu conteaza, se sterg la compilare). Datoria cunoscuta (service locator-ele din `infra/redis/redisCacheContext.ts`, `infra/redis/redisContext.ts`, `sources/sourceRegistry.ts` si wiring-ul `commandRuntimeDependencies.ts`/`operationJournalRuntime.ts`) e intr-un allowlist explicit legat de refactorizarile din review (Major #1/#2/#8): orice fisier NOU cu aceeasi problema pica la CI, iar cand datoria se rezolva, intrarea se scoate din allowlist.
