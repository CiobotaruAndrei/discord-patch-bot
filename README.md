...
- `/set mode | mindiscount | maxprice | free | paid | currency | stores | update-template | discount-template` - (admin) configurari de afisare/filtrare per-server. `update-template` si `discount-template` personalizeaza mesajul-text de deasupra embed-urilor la notificarile de update, respectiv reduceri (placeholder `{count}` = numarul de elemente; gol = revine la implicit).
...
- `/sources refresh` - (admin) cu optiunea `game:<key>`, forteaza un fetch live pentru acel joc si afiseaza rezultatul brut (titlu/link sau eroare/outcome). E inspectie: nu marcheaza update-ul ca vazut si nu trimite notificari; are cooldown.
