...
- Sanatatea outbox-ului este expusa la `/metrics`: `bot_outbox_sent`, `bot_outbox_retried`, `bot_outbox_dead_lettered`, `bot_outbox_drains`, `bot_outbox_queue_depth`, `bot_outbox_delivery_ms_total` (latenta cumulata; impreuna cu `bot_outbox_sent` da latenta medie), `bot_outbox_oldest_job_age_seconds` (vechimea celui mai vechi job in coada), `bot_outbox_lock_acquire_failures` si `bot_outbox_mark_sent_failures` (livrari care nu au putut fi marcate in istoricul de dedupe -> risc de re-trimitere la recovery). Coada `notificationOutbox` are si un index unic *sparse* pe `dedupeKey`, deci doua joburi pending cu acelasi continut nu pot coexista. Cand un drain raporteaza esecuri de citire a istoricului (recovery-verify) sau de marcare, worker-ul trimite si un admin alert (cu cooldown) ca operatorul sa afle proactiv.
