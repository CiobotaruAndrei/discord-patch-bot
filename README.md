...
- `BOT_ROLE` - `all` (implicit), `web` sau `worker`. `all` = un singur proces care face tot (comportamentul actual). `web` = doar interactiuni Discord + slash commands + HTTP, fara job-uri de fundal. `worker` = doar job-urile de fundal (cron, drain outbox, housekeeping) + HTTP health/metrics, fara sa trateze interactiuni. Cele doua roluri se pot rula ca procese separate (`npm start` / `npm run start:worker`) si se coordoneaza prin lock-urile DB. **Setul de intents Discord se deriva din rol**, prin aceeasi conditie care decide ce listeneri se inregistreaza: `web` si `all` primesc tot setul, iar `worker` primeste doar `Guilds`, cat ii trebuie ca sa rezolve canalele in care trimite. Asa un worker nu mai cere `GuildMembers` si `MessageContent`, care sunt intents privilegiate, pentru evenimente pe care nu le asculta. Vezi sectiunea [Worker separat](#worker-separat).
...
- `NATIVE_INSPECTOR_ISOLATION` - unde ruleaza inspectia de continut netrusted: `auto` (implicit), `on` sau `off`. Pe `auto`, inspectia ruleaza in procesul separat cu filtru de syscall **doar in productie Linux** (`NODE_ENV=production` + `process.platform === "linux"`) si doar daca binarul `native-inspector` exista; in rest ramane addon-ul in-proces. `on` forteaza procesul izolat oriunde exista binarul (fara filtru seccomp pe alte platforme), `off` il opreste complet. Cand ruta izolata e activa si procesul nu produce verdict (ucis, termen depasit, raspuns necitibil), inspectia **nu** se intoarce la addon-ul in-proces, ci cade pe parserul TypeScript: un input care doboara parserul nativ in sandbox nu trebuie rulat prin acelasi parser nativ fara sandbox.
- `NATIVE_INSPECTOR_PROCESSES` - cate procese de inspectie ruleaza in paralel cand ruta izolata e activa (implicit `2`, plafon `8`). Protocolul e o cerere per proces, deci numarul de procese e si limita de atasamente inspectate simultan.
- `NATIVE_INSPECTOR_BINARY` - cale explicita catre binarul `native-inspector`. Fara ea, se cauta langa addon (`native/native-inspector`), apoi in `native/target/release/`.
...
Conventia `:prebuilt` e generala, nu doar pentru `check`: **orice** script care ruleaza cod compilat are o pereche — `X` construieste si deleaga, `X:prebuilt` ruleaza pe `dist`-ul existent. Comanda reala traieste intr-un singur loc, in varianta `:prebuilt`, deci nu se poate desincroniza de varianta cu build. Workflow-urile de CI construiesc o singura data per job si cheama mai departe numai variantele `:prebuilt`: `ci.yml` construieste prin `check` si ruleaza apoi `benchmark:guard:prebuilt` si `test:e2e:prebuilt`; `release.yml` construieste prin `check:full` si ruleaza `canary:sources:prebuilt`; `canary.yml` si `staging-smoke.yml` au un pas explicit de build, dupa care probele ruleaza prebuilt. Gate-ul `prebuiltScriptPolicy.test.ts` respinge un script care construieste si ruleaza `dist` in aceeasi comanda, un `:prebuilt` care construieste totusi ceva, un job de workflow care compileaza TypeScript de doua ori si o cale catre `dist` scrisa direct in YAML.
...
Gate-ul de straturi (`scripts/check-layer-imports.ts`, rulat de `npm run check` si de CI) **nu mai are niciun mecanism de exceptie**: nu exista allowlist, waiver sau lista de fisiere scutite, deci o incalcare e raportata indiferent de cine o produce. Mecanismul a existat cat timp exista datorie de wiring (`commandRuntimeDependencies.ts`, `operationJournalRuntime.ts`, apoi `sources/sourceRegistry.ts` si locatorii Redis din `infra/redis/`); dupa ce toate au fost rezolvate — motorul de jurnal traieste acum in `shared/`, iar instantele de surse si Redis in composition root — allowlist-ul a fost **sters**, nu golit. Gardat de `layerGateWithoutExceptions.test.ts`, care verifica pe cod ca gate-ul nu are cum sa scuteasca un fisier, si ca un import de **tip** pe aceeasi cale ramane permis (nu creeaza dependinta la runtime).

`npm run check` ruleaza si `check:weakening` (`scripts/check-no-weakening-types.ts`), care **esueaza** daca exista constructii care **slabesc tiparea** in codul sursa (`.ts`/`.js`, inclusiv `src/test/`), conform regulii 2: `any`, `as never`, sau dubla asertiune `as unknown as`. Verificat pe AST (TypeScript), nu pe text, deci nu da fals pozitiv pe string-uri. **NU** sunt interzise `unknown` (tipul top, type-safe, opusul lui `any`) si nici casturile de **narrowing** care ingusteaza din `unknown`/date dinamice la un tip utilizabil (ex. `value as Record<string, unknown>`, `item as DealInfo`, `require(...) as typeof import(...)`) — acelea intaresc tiparea, nu o slabesc. Exceptia regulii 2 este stricta: testele pot contine constructii deliberate care slabesc tiparea doar cand fisierul este in allowlist-ul explicit pentru teste bug-catching, in prezent `src/test/gates/checkNoWeakeningTypes.test.ts`.
...
Granitele tiparii — ce ramane intentionat mai lax (ca afirmatiile de mai sus sa fie verificabile, nu doar optimiste): (1) adaptoarele `attachX(target)` mai exista (12 module Mongo/shared plus cateva din `features/`), fiindca scripturile si testele de integrare isi compun singure contextul — dar **punga** de pe input-ul lor a disparut: niciun target nu mai e `XDeps & Record<string, unknown>`, ci o interfata declarata, iar compunerea de productie merge doar prin `buildFrom(context)`; (2) `commandRegistry` se compune acum **imutabil** prin `createAppServices` (fiecare zona obtinuta prin spread in obiecte noi, fara mutatie in-place a unui `base` partajat), are un **contract de export inchis** (`RequiredCommandRegistry`) verificat de `tsc`, iar registrul public returnat e `Object.freeze`-uit; `sourceRegistry` (`createSourceRegistry(deps: SourceRuntimeDeps): SourceRegistryApi`) compune **explicit**: `createSourceRuntime(deps)` -> `attachHttpClient.buildFrom(runtime)` -> `createSteamSource({...})` -> `createUpdates({...})` -> `createDeals({...})`, fiecare fabrica primind un obiect literal cu dependintele enumerate, iar registrul returnat e un literal `Object.freeze`-uit cu tip de retur inchis (o dependinta lipsa e eroare de compilare, nu throw la boot); boundary-ul dinamic `installers: unknown[]` a fost eliminat din ambele; (3) **payload-urile dinamice** (raspunsuri de API extern, `Schema.Types.Mixed`, datele dintre handler-e si embed builders) sunt tipate `unknown` si ingustate la consum — `unknown` e alegerea sigura aici, nu o lipsa; (4) restul `Record<string, unknown>` din handler-e sunt obiecte de date construite local (embed-uri, optiuni), nu contracte de dependinte. Modelele Mongo din `mongoContext` au tipuri de document dedicate (`infra/mongo/modelTypes.ts`, gard in `mongoContextTypedApi.test.ts`); in tot codul runtime nu mai exista `: any` (verificabil cu `grep -rn ": any" --include="*.ts"` pe directoarele runtime).
...
- Imaginea Docker e scanata cu **Trivy** (vulnerabilitati CRITICAL/HIGH, `ignore-unfixed`) si genereaza un **SBOM CycloneDX** prin workflow-ul `container-scan.yml` (push pe `main` cand se schimba Dockerfile/dependintele, **pe fiecare `pull_request` catre `main`**, saptamanal si manual). Pe PR ruleaza un pas-poarta Trivy cu `exit-code: 1` care **blocheaza merge-ul** daca imaginea are o vulnerabilitate fixabila CRITICAL/HIGH (la fel ca `check`); rezultatele Trivy apar in tab-ul Security (cod scanning, pe push/schedule), iar SBOM-ul e artifact. Jobul face **doua** treceri de Trivy, nu trei: SARIF-ul si pasul-poarta cer exact acelasi set de vulnerabilitati si difera doar prin format, deci pornesc dintr-un singur raport JSON, redat local cu `trivy convert`. SBOM-ul ramane scanare separata dinadins, fiindca `trivy convert` accepta `--severity` dar nu are `--ignore-unfixed`: dintr-un raport deja filtrat, inventarul si-ar pierde vulnerabilitatile fara fix. Fiecare pornire de Trivy isi descarca din nou baza de vulnerabilitati, masurat 14-15s, de unde vine economia. Completeaza CodeQL + Dependency Review (analiza de cod + dependinte) cu scanarea imaginii (supply chain). La release, imaginea publicata pe GHCR trece prin acelasi gate Trivy blocant **pe imaginea exacta**: build local (fara push), scanare, apoi `docker tag` + `docker push` pe bytes-ii scanati — nu exista cale de publicare nescanata. Tot la release ruleaza si canary-ul live pe surse (fail-closed pe API-urile fiabile) pe codul exact al tag-ului, prin `npm run canary:sources:prebuilt`, adica pe `dist`-ul deja construit de `check:full`.
