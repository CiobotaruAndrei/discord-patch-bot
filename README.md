...
- `TRUST_PROXY` / `TRUSTED_PROXY_COUNT` - cand botul ruleaza in spatele unui reverse proxy/LB, seteaza `TRUST_PROXY=true` ca rate limiter-ul sa ia IP-ul clientului din `X-Forwarded-For`. `TRUSTED_PROXY_COUNT` (implicit `1`) = cate proxy-uri trusted ai in fata botului; IP-ul clientului e luat ca `segments[length - TRUSTED_PROXY_COUNT]` (sare hop-urile tale trusted din dreapta, deci spoofing-ul leftmost e ignorat). Mareste-l (ex. `2` pentru `CDN -> LB -> bot`) ca toti clientii din spatele aceluiasi proxy sa NU fie grupati pe acelasi IP. Un lant XFF mai scurt decat valoarea cade pe IP-ul socket-ului (anti-truncare).
