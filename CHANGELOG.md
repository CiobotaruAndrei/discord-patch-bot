...
- Versionarea hash-urilor de deduplicare (`HASH_VERSION`): guild-urile stocheaza `seenHashVersion` si `discountsHashVersion`. Daca algoritmul de hash (`stableUpdateId`/`dealHash`) se schimba, cron-ul face un re-baseline silentios (re-seed `seen`/`seenDiscounts` cu elementele curente, fara notificari) in loc sa retrimita totul ca „nou". `/start` seteaza versiunea la baseline, o migrare (`m5`) o completeaza pentru guild-urile existente, iar teste functionale acopera re-baseline-ul pentru update-uri si reduceri. Previne spam-ul masiv de notificari la o schimbare de algoritm.
- Metrici Prometheus etichetate per-sursa pe `/metrics`, pentru debugging pe sursa si pe tip de eroare: `bot_source_fetch_success_total{source}`, `bot_source_failures_total{source,reason}` (motivul e codul HTTP, codul de retea sau `error`/`cb_read`) si `bot_scrape_schema_drift_total{source}`. Instrumentate in `executeFetchWithCircuitBreaker`, expuse alaturi de contoarele globale existente.
- Smoke manual de staging (`npm run smoke:staging` + workflow `staging-smoke.yml` rulat manual): conecteaza Mongo, ruleaza migrarile, se logheaza la Discord, inregistreaza comenzile pe `DISCORD_DEV_GUILD_ID`, fetch-uieste o sursa Steam si trimite un mesaj in `DISCORD_TEST_CHANNEL_ID`. Logica e un `runStagingSmoke(deps)` cu dependinte injectate (testat cu mock-uri pentru fiecare pas si pentru oprirea la primul esec), iar entrypoint-ul cu module reale ruleaza doar cand fisierul e executat direct. Se ruleaza inainte de release, nu pe fiecare PR, si foloseste secrete GitHub `STAGING_*`.
