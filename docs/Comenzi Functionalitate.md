...
| `/security-log` | Admin, Ephemeral, Guild-only | Afiseaza paginat cronologia unica a incidentelor tuturor protectiilor, cele mai recente primele, cu token-urile, linkurile si ID-urile redactate. Filtrarea dupa sursa restrange lista fara sa schimbe ordinea. |
| `/security-status` | Admin, Ephemeral, Guild-only | Arata pentru fiecare protectie si pentru cele sase subprotectii moderation-guard daca este pornita, oprita, incompleta (fara canal) sau degradata (lipsesc permisiuni), plus aprobarile active, resursele protejate degradate si etapa incidentului anti-raid. |
