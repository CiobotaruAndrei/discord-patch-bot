...
| `/start threat-protection` | Admin, Ephemeral, Guild-only | Inspecteaza continutul, redirecturile HTTP sigure, linkurile si atasamentele prin MIME si semnaturi de fisier. Verdictele sunt separate pe categorii: `confirmed` (malware confirmat) se sterge automat; `risky-file` (executabil/script confirmat DOAR ca tip de fisier) si `policy-violation` (@everyone/@here, invitatii Discord) raman implicit pe canal si produc doar alerta cu categoria explicita; documentele, arhivele sau resursele neverificabile raman `uncertain`, cu alerta pentru verificare manuala. |
| `/stop threat-protection` | Admin, Guild-only | Dezactiveaza detectarea si pastraza canalul configurat. |
| `/start threat-delete-risky-files` | Admin, Ephemeral, Guild-only | Politica explicita a serverului: mesajele cu fisiere executabile sau script detectate prin continut se sterg automat. Tipul fisierului e confirmat prin semnatura, nu si intentia malware — de activat doar pe servere care nu asteapta astfel de fisiere. Necesita `threat-alert-channel` configurat. |
| `/stop threat-delete-risky-files` | Admin, Guild-only | Revine la comportamentul implicit: fisierele riscante doar alerteaza, fara stergere automata. |
| `/start threat-delete-policy-violations` | Admin, Ephemeral, Guild-only | Politica explicita a serverului: mesajele cu @everyone/@here sau invitatii Discord externe se sterg automat. Sunt incalcari de politica, nu amenintari informatice. Necesita `threat-alert-channel` configurat. |
| `/stop threat-delete-policy-violations` | Admin, Guild-only | Revine la comportamentul implicit: incalcarile de politica doar alerteaza, fara stergere automata. |
