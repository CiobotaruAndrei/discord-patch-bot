...
| `/set update-template value:<text>` | Admin, Ephemeral | Personalizeaza mesajul (text deasupra embed-urilor) trimis la notificarile de update. `{count}` = numarul de update-uri din mesaj. Fara valoare = revine la implicit (doar mentiunea de rol, daca e configurata). |
| `/set discount-template value:<text>` | Admin, Ephemeral | La fel pentru notificarile de reduceri. `{count}` = numarul de reduceri. Fara valoare = revine la implicit. |
...
| `/sources refresh game:<key>` | Admin, Ephemeral | Forteaza un fetch live pentru un joc si arata rezultatul brut: titlu si link daca s-a gasit un update, sau eroarea/outcome-ul daca sursa a esuat. Este inspectie: nu marcheaza update-ul ca vazut si nu trimite notificari. Are cooldown (fetch-ul e costisitor). |
