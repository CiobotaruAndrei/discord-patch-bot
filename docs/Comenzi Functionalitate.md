...
| `/admin-access unlock code:<cod>` | Public, guild-only, Ephemeral | Activeaza pentru userul curent un grant admin temporar folosind un cod din `BOT_ADMIN_ACCESS_CODES`. Codul nu se salveaza in Mongo; se salveaza doar userul, serverul si expirarea grantului. Grantul expira dupa 30 de minute. |
| `/admin-access status` | Public, guild-only, Ephemeral | Arata daca userul curent are un grant admin activ prin cod pe serverul curent si cand expira. |
| `/admin-access revoke` | Public, guild-only, Ephemeral | Revoca grantul admin prin cod pentru userul curent pe serverul curent. |
| `/set admin-command-access role:<rol> mode:<role|role-or-higher>` | Owner-only, Ephemeral | Seteaza conditia de rol pentru folosirea comenzilor admin. `role` accepta doar rolul ales, iar `role-or-higher` accepta rolul ales sau un rol mai mare in ierarhia Discord. Pana ownerul seteaza aceasta regula, rolurile nu dau acces admin; raman doar `Administrator` si granturile temporare prin cod. |
...
| `/delete admin-command-access confirm:true` | Owner-only, Ephemeral | Sterge regula de rol pentru comenzile admin si revine la accesul implicit: `Administrator` sau grant temporar prin cod. |
