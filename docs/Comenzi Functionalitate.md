...
| `/set permission-request-channel canal:<canal>` | Admin, Ephemeral, Guild-only | Configureaza canalul unic pentru toate cererile de aprobare de securitate si pentru deciziile ownerului, cu validarea permisiunilor View Channel, Send Messages si Embed Links. Preia si fluxul de aprobare pentru adaugarea botilor: la migrare, valoarea vechiului canal de alerte bot-add este copiata aici, iar campurile vechi sunt sterse din documentul guild. |
| `/set anti-raid-alert-channel canal:<canal>` | Admin, Ephemeral, Guild-only | Seteaza canalul anti-raid in care se publica alertele, interventiile, participantii, sanctiunile si erorile incidentului. Daca nu este setat, botul foloseste canalul din `/set permission-request-channel`. |
| `/set anti-raid-thresholds <optiuni>` | Admin, Ephemeral, Guild-only | Modifica pragurile anti-raid. Implicit: 3 mesaje identice sau aproape identice in 8s; minimum 4 mentiuni in 10s (intra `@everyone`, `@here`, rolurile si utilizatorii, cu sau fara text); minimum 3 mesaje cu servere, invitatii sau reclame in 20s; minimum 4 mesaje cu linkuri sau atasamente in 12s; minimum 2 persoane sau boti care participa coordonat in 15s; minimum 3 canale sau roluri create ori sterse fara autorizatie in 20s; perioada de siguranta 30m; mute 24h; timeout 24h; lockdown maxim 45m inainte de solicitarea interventiei ownerului. Nu exista un prag separat pentru spam general. Optiunile nedate raman la valoarea curenta, iar o valoare in afara limitelor este refuzata cu motiv, fara sa anuleze celelalte valori valide din aceeasi comanda. |
| `/set ad-alert-channel canal:<canal>` | Admin, Ephemeral, Guild-only | Seteaza canalul pentru cererile de aprobare a reclamelor, reclamele sterse, tentativele detectate si warn-urile automate. |
| `/start ad-protection` | Admin, Ephemeral, Guild-only | Porneste protectia impotriva reclamelor neaprobate de owner, **inclusiv reclamele fara link**: promovarea altor servere Discord, comunitati, servicii, produse, pagini sau conturi. Prima tentativa sterge mesajul si devine 1/3; a doua devine 2/3 si avertizeaza ca urmatoarea produce warn; a treia adauga automat un warn in sistemul existent, vizibil in `/warn-list`, iar atingerea limitei din `/warn-ban-limit` produce ban automat. Dupa warn contorul revine la 0/3, iar istoricul ramane salvat. Ownerul poate publica reclame direct, iar reclamele aprobate nu cresc contorul. |
| `/stop ad-protection` | Admin, Ephemeral, Guild-only | Opreste protectia si transforma toate cererile si aprobarile active neexpirate in `cancelled`. Istoricul tentativelor, warn-urile si canalul configurat raman salvate. |
| `/ad-request reclama:<text> atasament:<fisier>` | Public, Ephemeral, Guild-only | Permite unui utilizator sa ceara aprobarea ownerului inainte sa publice o reclama. Cererea salveaza utilizatorul, reclama exacta, linkul, invitatia, atasamentul, tinta promovata si expirarea. Ownerul primeste cererea in canalul configurat, cu butoanele Aproba si Respinge. Aprobarea e valabila numai pentru reclama si utilizatorul din cerere, se consuma o singura data si expira; o modificare semnificativa a reclamei o invalideaza. |
| `/ad-permissions list` | Owner-only, Ephemeral, Guild-only | Afiseaza cererile si aprobarile pentru reclame, cu ID-ul cererii, utilizatorul, rezumatul reclamei, statusul (`pending`, `approved`, `rejected`, `used`, `expired`, `cancelled`), data solicitarii, decizia ownerului, expirarea si folosirea aprobarii. Cererile active apar inaintea istoricului. |
| `/ad-attempts list utilizator:<membru>` | Admin, Ephemeral, Guild-only | Afiseaza tentativele active 0/3, 1/3 sau 2/3, totalul reclamelor sterse, warn-urile automate, ultima tentativa, canalul incidentului si istoricul grupurilor de trei tentative. Tentativele sunt salvate atomic, astfel incat mesajele trimise rapid sa nu poata evita warn-ul. |
| `/anti-raid status` | Admin, Ephemeral, Guild-only | Afiseaza ID-ul incidentului, etapa (`suspectat`, `confirmat`, `izolare`, `curatare`, `restaurare`, `inchis`), canalele blocate in acest moment, numarul participantilor opriti, al celor nesanctionabili si al celor ramasi, durata lockdown-ului masurata din confirmare, timpul ramas din perioada de siguranta, progresul restaurarii, operatiunile ramase si ultimele erori. |
| `/anti-raid participant-list <optiuni>` | Admin, Ephemeral, Guild-only | Afiseaza participantii incidentului activ, ai ultimului raid sau ai incidentului indicat, cu sanctiunile aplicate, cele esuate si ultima eroare. Un incident care apartine altui server nu poate fi citit. |
| `/anti-raid force-start` | Owner-only, Ephemeral, Guild-only | Confirma manual un raid, genereaza un ID de incident si porneste interventia. Nu se poate deschide un al doilea incident peste unul activ. |
...
| `/protected-resource action:<add\|remove\|list> type:<channel\|category\|role> target:<resursa>` | Owner-only, Ephemeral, Guild-only | Marcheaza canale, categorii si roluri ca resurse critice. `add` salveaza snapshot-ul complet (nume, pozitie, categorie parinte, permisiuni si overwrite-uri) si evalueaza pe loc daca prevenirea poate fi garantata; `remove` scoate resursa din protectie fara sa o stearga; `list` arata resursele, starea snapshot-ului si cauzele exacte pentru cele `degraded`, cu ele afisate primele. Aplicarea in afara raidurilor porneste doar cand `/start moderation-guard` este activ. O resursa devine `degraded` cand prevenirea nu poate fi garantata: roluri sau membri cu Administrator ignora overwrite-urile canalului, rolul protejat este sub un rol care il poate administra, rolul botului nu este deasupra resursei, ori botul nu are View Audit Log si atunci autorul nu poate fi confirmat. La stergere, resursa se recreeaza din snapshot; ID-ul nou este legat de cel vechi, dar mesajele, invitatiile si referintele vechi nu se pot recupera. Cat timp `/start moderation-guard` este activ si nu exista raid confirmat, orice stergere, redenumire, mutare, schimbare de pozitie sau modificare de permisiuni a unei resurse protejate trece doar daca autorul e ownerul ori daca are o aprobare activa de tip `protected-resource-change`; altfel botul confirma autorul prin Audit Log, restaureaza din snapshot, ii elimina rolurile cu permisiuni ridicate pe care le poate administra si publica in canalul configurat tagul, motivul, resursa si lista exacta a rolurilor eliminate, inclusiv cele care NU au putut fi eliminate. Daca Audit Log-ul nu confirma autorul, nu se aplica nicio sanctiune. |
| `/start anti-raid` | Admin, Ephemeral, Guild-only | Porneste protectia anti-raid. Refuza activarea daca botului ii lipsesc View Audit Log, Moderate Members, Mute Members, Manage Channels sau Manage Roles ori nu are un rol deasupra @everyone, fiindca fara ele nu poate sanctiona si nu poate bloca canale. |
| `/stop anti-raid confirm:<true>` | Admin, Ephemeral, Guild-only | Opreste modulul anti-raid. Owner-only si cu confirmare obligatorie, fiindca oprirea lasa serverul neprotejat; este diferita de `/anti-raid force-stop`, care doar incheie un incident in curs. Oprirea cere in plus un incident activ intr-o etapa de interventie (`confirmed`, `containment`, `cleanup`, `recovery`): fara raid tratat, comanda refuza si spune de ce, ca sa nu devina un comutator general de dezactivare. Daca starea incidentelor nu poate fi citita, comanda refuza tot - o baza de date inaccesibila nu e o dovada ca nu exista raid. |
| `/start moderation-guard` | Admin, Ephemeral, Guild-only | Porneste unitar, in afara raidurilor, cele sase protectii administrative bazate pe aprobare: `bot-add`, `permission-grant`, `moderation-mass`, `webhook`, `server-structure` si `protected-resource-change`. Cere canalul din `/set permission-request-channel` cu permisiunile View Channel, Send Messages si Embed Links. Cu poarta pornita, o operatiune sensibila trece doar daca autorul e ownerul sau daca are o aprobare exacta si nefolosita; altfel se aplica regula corectiva comuna. Pentru `server-structure`, corectia inseamna si revenirea efectiva: canalul sau rolul creat fara aprobare e sters, iar cel sters e recreat din snapshot, cu verificarea rezultatului si raportarea a ce nu a putut fi reparat. Cat timp un raid este confirmat, poarta se suspenda, ca sa nu se suprapuna peste anti-raid. |
| `/stop moderation-guard` | Admin, Ephemeral, Guild-only | Opreste poarta si anuleaza intai toate cererile si aprobarile nefolosite pentru cele sase tipuri, apoi stinge protectia, ca o aprobare veche sa nu poata fi consumata dupa o repornire. Canalul si istoricul raman salvate. Cu poarta oprita, retragerea automata a permisiunilor sensibile nu mai ruleaza ca mecanism permanent independent. |
| `/security-log` | Admin, Ephemeral, Guild-only | Afiseaza paginat cronologia unica a incidentelor tuturor protectiilor, inclusiv auditul serverului, incidentele anti-raid, aprobarile administrative, aprobarile pentru reclame si tentativele de reclama. Cele mai recente apar primele, token-urile, linkurile si ID-urile sunt redactate, iar fiecare raspuns este limitat sigur la 2.000 de caractere. Filtrarea dupa sursa restrange lista fara sa schimbe ordinea. Daca persistenta nu poate fi citita, comanda raporteaza indisponibilitatea in loc sa afiseze un jurnal fals gol. |
| `/security-status` | Admin, Ephemeral, Guild-only | Arata pentru fiecare protectie si pentru cele sase subprotectii moderation-guard daca este pornita, oprita, incompleta (fara canal) sau degradata. Starea degradata este calculata din permisiunile Discord live ale botului, inclusiv accesul la canalul configurat si permisiunile necesare protectiei. Raportul include aprobarile active, resursele protejate degradate, etapa incidentului anti-raid si operatiunile de recovery care cer interventia ownerului. Daca MongoDB nu poate fi citit, comanda spune explicit ca starea nu este disponibila. |
| `/permission-request type:<bot-add\|permission-grant\|moderation-mass\|webhook\|server-structure\|protected-resource-change> target:<resursa> action:<actiune> reason:<text>` | Public, Ephemeral, Guild-only | Singura comanda pentru toate cererile de aprobare de securitate. Optiunile care nu se aplica tipului ales sunt ignorate. Cererea ajunge in canalul din `/set permission-request-channel`, cu ownerul mentionat si butoanele Aproba si Respinge. La Aproba, ownerul poate restrange tinta, actiunea, cantitatea, permisiunile si valabilitatea printr-un formular; aprobarea ramane legata exact de solicitant si de operatiune, expira si se consuma o singura data. |
| `/permission-requests list status:<optional> type:<optional>` | Owner-only, Ephemeral, Guild-only | Afiseaza intr-o singura lista toate cererile create prin `/permission-request`, cu ID, tip, solicitant, bot, tinta, actiunea, cantitatea, permisiunile, valorile aprobate de owner, motivul, expirarea, ownerul care a decis, statusul si momentele. Cererile active apar inaintea istoricului. |
