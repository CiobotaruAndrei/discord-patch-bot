...
- Instanta unica `redisRuntime` (si wrapper-ul `redisCache`) e construita in composition root (`src/app/runtimeComposition.ts`) din `env`+`logger` si partajata intre boot/shutdown si contextul de comenzi, ca toata aplicatia sa vada aceeasi conexiune. Fostele module-locator `infra/redis/redisContext.ts` si `infra/redis/redisCacheContext.ts` (care importau instanta INAPOI din `app` — inversare de straturi) au fost STERSE (review nou, Major #1): consumatorii iau instantele direct din composition root, iar gate-ul `check:layers` interzice orice import `infra -> app` fara exceptii.
