...
- Contextul de instalare al fiecarei familii de surse este acum exact contractul de intrare al factory-ului, fara `Partial<Api>` si fara escape-hatch `Record<string, unknown>`: `SteamSourceContext = SteamSourceDeps`, `DealsContext = DealsDeps`, `UpdatesContext = UpdatesDeps`. Fiecare `build*From(target)` citeste doar campurile declarate in deps, iar API-ul returnat este compus separat prin spread intr-un obiect nou. Garda in `sourceInstallerTargetContracts.test.ts` pinuieste cele trei contracte explicite.
...
- `updatesContracts.ts` — tipurile partajate (`CircuitBreakerDoc`, `UpdatesDeps`, `UpdatesContext = UpdatesDeps`).
