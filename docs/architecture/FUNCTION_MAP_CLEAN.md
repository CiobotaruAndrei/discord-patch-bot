...
### `src/features/notifications/futureReleaseNotifications.ts`

- **Motorul de decizie al automatizarii future-release** (audit de conformitate, #11, parte livrata): `computeFutureReleaseUpdate(observation, state, now)` e o functie PURA care, dintr-o observatie (`gameName`, `releaseDate`, `preorderPrice`) si starea persistata per joc, intoarce notificarile de emis + starea urmatoare. Semantica: la BASELINE (prima procesare) nu emite nimic si marcheaza pragurile calendaristice deja trecute ca notificate (activarea nu trimite un val cu informatiile existente); apoi emite pragurile 30/7/1 zile **cel mult o data fiecare** (cand `daysUntilRelease <= prag` si nu a fost deja notificat), evenimentul `preorder-available` la aparitia preorder-ului, `price-changed` cu `from`/`to` la schimbarea pretului, si `preorder-removed` (eveniment SEPARAT) la disparitia lui. `parseReleaseTimestamp` intoarce `null` pentru date doar-an sau invalide (fara praguri calendaristice, dar preorder/pret functioneaza). **Parte livrata (regula 24)**: jobul periodic care cheama acest motor pentru cele max 20 de jocuri ale serverului, fetch-ul live al datei/preorder-ului si persistenta starii + `activationId`/`initializing` la start/stop depasesc impreuna 2000 de linii si raman pas separat; motorul pur (partea cu cea mai mare densitate de bug-uri) e complet testat in `futureReleaseNotifications.test.ts`.
...
- Inspecteaza continutul mesajelor, redirecturile si atasamentele dupa MIME si semnaturi de fisier. **Verdictele sunt separate pe categorii** (raport post-#699, problema #3): `confirmed` = malware confirmat (rezervat unui motor antivirus/reputatie; semnaturile de fisier NU il produc) — singurul care se sterge automat neconditionat; `risky-file` = executabil/script confirmat prin continut DOAR ca tip de fisier, nu ca intentie (un instalator legitim nu mai e sters automat); `policy-violation` = @everyone/@here si invitatii Discord — incalcari de politica, nu amenintari informatice; `uncertain` = neinspectabil/arhive/documente. Stergerea automata pentru `risky-file`/`policy-violation` cere politica EXPLICITA per server: flagurile `threatAutoDeleteRiskyFiles`/`threatAutoDeletePolicyViolations` (implicit false), comutate prin `/start|/stop threat-delete-risky-files` si `/start|/stop threat-delete-policy-violations` (cer `threat-alert-channel` configurat). Alerta include categoria si severitatea onesta ("neconfirmata ca malware", "incalcare de politica a serverului"). **Verdictul `confirmed` (malware confirmat) NU e produs de euristici** (raport post-#705, #2): detectarea semnaturii confirma tipul de fisier, nu intentia malware. `confirmed` e rezervat unui motor extern de reputatie/antivirus, injectat optional prin `reputationScan` in `ThreatInspectionDeps` — cand e configurat si raporteaza `"malware"` pentru o resursa inspectata, verdictul escaladeaza la `confirmed` (si abia atunci se declanseaza stergerea automata neconditionata); cand NU e configurat (situatia curenta din runtime, `reputationEngineConfigured` = false), `confirmed` nu e produs deloc, iar stergerea automata a fisierelor riscante / incalcarilor de politica ramane exclusiv pe opt-in-ul explicit al serverului. Un motor de reputatie cazut sau care raporteaza `clean`/`unknown` nu degradeaza verdictul euristic de baza. **Analiza pasiva de documente/arhive** (audit de conformitate, #20, parte livrata; recursivitatea completa de arhive + urmarirea lanturilor de redirect raman pentru motorul extern #21 fiindca depasesc 2000 de linii - regula 24): fara a EXECUTA continutul, se detecteaza OLE compound (`D0CF11E0`) ca document, indicatori de macro VBA (`vbaProject.bin`/`word/vbaProject`) si de script/actiune automata in PDF (`/JavaScript`, `/OpenAction`, `/Launch`) - toate raman `uncertain` (semnalate in reason, niciodata sterse fara confirmare); arhivele criptate (bit 0 din general purpose flag ZIP) raman `unknown`/`uncertain`, NU `dangerous` automat. **Analiza mesajului e COMPLETA, fara oprire la prima incalcare** (raport post-#705, #3): incalcarile de politica NU mai scurtcircuiteaza inspectia — toate componentele (ambele reguli de politica + toate linkurile si atasamentele) sunt evaluate, verdictul final e cel mai sever (`confirmed` > `risky-file` > `policy-violation` > `uncertain`), `reason` pastreaza TOATE motivele relevante (deduplicate, unite cu `;`), iar `detectedVerdicts` expune toate categoriile gasite; stergerea automata cu opt-in se decide pe ORICARE categorie detectata, nu doar pe verdictul cel mai sever (un mesaj cu @everyone + executabil se sterge cand serverul are opt-in pe incalcari de politica, chiar daca verdictul raportat e `risky-file`).
...
- **Boturile monitorizate sunt inspectate si dupa join** (audit de conformitate, #27, parte livrata): `handleMessageCreate` nu mai ignora autorii-bot cand `botAddProtectionEnabled` - mesajele si atasamentele lor trec prin aceeasi inspectie threat; un verdict `confirmed` (via motorul extern) => INCIDENT CRITIC urgent cu tag owner + stergere + audit `bot-confirmed-dangerous-activity`, iar un verdict neconfirmat (risky-file/policy-violation/uncertain) => alerta owner FARA stergere si FARA eliminare (un bot aprobat nu e eliminat automat pentru suspiciune). Contextul persistent de observatie per-bot si corelarea completa a actiunilor din Audit Log (roluri/canale/webhook/ban) intr-o fereastra raman pas separat (regula 24, >2000 linii cu starea persistata).
