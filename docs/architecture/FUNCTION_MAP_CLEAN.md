...
- Contextul de instalare al fiecarei familii de surse e tipat INGUST ca `Deps & Partial<Api>`, fara escape-hatch `Record<string, unknown>` (review impact-mare #7): `SteamSourceContext = SteamSourceDeps & Partial<SteamSourceApi>`, la fel ca `DealsContext = DealsDeps & Partial<DealsApi>` si `UpdatesContext = UpdatesDeps & Partial<UpdatesApi>`. Fiecare `build*From(target)` citeste DOAR campurile declarate in `*Deps` (steam: `logger`/`getCurrencyConfig`/`httpReq`/`safeCheerioLoad`), iar `Partial<*Api>` acopera contributiile stagiilor anterioare din compunere fara sa admita chei arbitrare — un typo pe o cheie de context nu mai trece de typecheck. Garda in `sourceInstallerTargetContracts.test.ts` (toate cele trei familii folosesc `Deps & Partial<Api>`, niciuna nu mai are `Deps & Record<string, unknown>`).
