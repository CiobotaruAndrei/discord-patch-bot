...
- Sub-schemele guild traiesc in module pe domenii: `guildNotificationSchemas.ts` (pendingUpdate/pendingDiscount/deadLetterEntry/priceAlert), `guildYoutubeSchemas.ts` (youtubeChannel/youtubeError/youtubeChannelRoute/youtubeLastError), `guildAdminRecordSchemas.ts` (configBackup/suggestedCommand/watchlistGameSuggestion/futureReleaseGame/adminCommandAccess). Auditul admin nu mai sta pe documentul guild: `auditLogSchemas.ts` defineste `guildAuditLogSchema` pentru colectia dedicata `guildAuditLogs` (`{ guildId, kind: "bot"|"server", userId, command, action, result, details, at }`), cu index `{ guildId, kind, at: -1 }` pentru listari si TTL pe `at` din `GUILD_AUDIT_LOG_TTL_DAYS` (implicit 180 de zile) in locul vechiului cap de 100 de intrari per array; migrarea de boot #8 muta intrarile existente (upsert idempotent pe continut) si sterge campurile vechi `botAuditLog`/`serverAuditLog`.
