...
- Invalidarea cache-ului setarilor guild este garantata la limita Mongo: middleware-ul `guildSchema` publica `GuildSettingsChanged` dupa scrierile reusite pe un guild, iar `guildSettings.ts` consuma evenimentul si elimina intrarea respectiva. Handler-ele nu mai sunt singura linie de aparare impotriva datelor stale; apelurile manuale ramase sunt compatibilitate redundanta si pot fi eliminate gradual.
