...
Gate-uri migrate: `registryClosedContracts.test.ts` (cel mai afectat — avea peste 100 de asertari textuale, inclusiv linii de compunere din `mongoContext` fixate caracter cu caracter), `metricRecorderBoundaries.test.ts`, `commandInstallerTargetContracts.test.ts`, `mongoPorts.test.ts`. Diferenta se vede la compunerea `mongoContext`: in loc de linia exacta `attachModelsModule.buildFrom(withUtilities)`, gate-ul verifica acum **proprietatea** — fiecare strat are exact un `buildFrom`, extinde strict stratul anterior si primeste ca argument un strat deja construit, niciodata unul viitor. Un strat nou trece fara sa atinga testul; o compunere care sare peste un strat sau citeste in avans pica.
