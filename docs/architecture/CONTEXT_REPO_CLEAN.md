...
## Snapshot arhitectural curent

- Contextul runtime al comenzilor este impartit in patru sectiuni explicite: Discord, Mongo, surse si platforma. Selectoarele au contracte `Pick` inchise, iar compatibilitatea cu handler-ele plate este izolata in `flattenCommandRuntimeDependencies`.
- Registry-ul de comenzi este declarativ prin `commandHandlerDescriptors`: fiecare intrare are ID stabil, domeniu si factory, iar duplicatele opresc boot-ul sau testele.
- Configuratia nu mai circula ca doua obiecte monolitice: `GuildSettings` si `RuntimeEnv` sunt compuse din contracte pe responsabilitati, iar schemele Mongo primesc numai `MongoModelEnv`.
- Incarcarea feed-urilor de update-uri si reduceri foloseste acelasi pipeline pentru fetch, persistenta snapshot, validare si fallback. Transformarea payload-ului si regulile de notificare raman in serviciul domeniului. Snapshot-urile cu itemi invalizi sunt respinse implicit (`reject-snapshot`), iar un rezultat partial se obtine doar prin optiunea explicita `drop-invalid`; un snapshot complet invalid nu este tratat ca feed gol.
- Operatiile administrative multi-document folosesc jurnalul cu stari `pending`, `leased`, `done`, owner unic, expirare si compare-and-set pe versiunea lease-ului. Executorii sunt idempotenti, iar auditul foloseste ID-ul operatiei pentru deduplicare.
- Outbox-ul persista `deliveryAcceptedAt` imediat dupa acceptarea livrarii Discord. Daca procesul cade inainte de istoric, markerul de deduplicare sau finalizare, reluarea completeaza numai persistenta si nu retrimite mesajul.
- Housekeeping foloseste `ScheduledTaskRunner` pentru non-overlap, abort si stare operationala observabila.
- Codul sursa si scripturile sunt ESM; exceptiile CommonJS descrise in sectiunile istorice de mai jos nu mai sunt valabile.
...
- Invalidarea cache-ului setarilor guild este garantata la limita Mongo: middleware-ul `guildSchema` publica `GuildSettingsChanged` dupa scrierile reusite pe un guild, iar `guildSettings.ts` consuma evenimentul si elimina intrarea respectiva. Handler-ele nu mai au NICIO invalidare manuala (review nou, Mediu #16): singura cale de invalidare e evenimentul `GuildSettingsChanged` emis de middleware-ul post-query DUPA scrierea comisa (`publishChangedGuild`, exportat si testat unitar in `guildSettingsCache.test.ts`); ultimele apeluri manuale (securityInteractionHandler) au fost eliminate, iar `invalidateGuildCache` nu mai e expus stratului de comenzi prin `commandRuntimeDependencies` (ramane API intern infra, abonat la event bus). Scrierile runtime pe Guild trec exclusiv prin `updateOne`/`findOneAndUpdate`/`deleteOne` (acoperite de hook-uri); migratiile de boot folosesc colectia bruta inainte ca vreun cache sa fie populat.
...
- Registrele de compunere (`commandRegistry`, `sourceRegistry`) folosesc importuri statice pentru module si contracte inchise la iesire (`CommandRegistryContext` fara index signature, `SourceRegistryApi` value-tipat); compozitia nu mai foloseste `as never`, `as unknown as` sau `LegacyInstallerTarget`. `commandRegistry` nu mai are mecanism de installers dinamici: compune explicit si **imutabil** prin factory-uri reale tipate (`createCommandCache`, `createCommandPresentation`, `createNotificationRuntime`, `createFeedbackRepository`, `createSlashCommandDefinitions`) intr-un `createAppServices` (fiecare zona prin **spread in obiecte noi**, fara `Object.assign(base, ...)` pe un singur obiect mutat; registrul public returnat e **`Object.freeze`-uit**), plus o **lista tipata `CommandHandler[]`** (din `attachX.buildCommandHandler(ctx)`) rutata de `dispatchCommand` (loop `canHandle`/`handle`), cu guard-urile compuse ca **pipeline explicit de functii** in registry (admin guard exterior -> snooze guard -> `dispatchCommand`, aceeasi ordine ca inainte), construite prin `createAdminCommandGuard`/`createCommandSnoozeGuard` — fara instalare prin mutarea contextului (registrul NU mai scrie nimic inapoi in context dupa compunere: `handleInteraction`/`buildHelpEmbed` sunt functii locale intoarse direct in obiectul inghetat, fara garda `requireInstalled`); `commandSnoozeGuard` si `adminCommandRouterGuard` sunt factory-only (obiecte inghetate de functii, installer-ul legacy apelabil a fost sters). Registry-ul intoarce contractul inchis `RequiredCommandRegistry` verificat de `tsc`; `sourceRegistry` (`createSourceRegistry(deps: SourceRuntimeDeps): SourceRegistryApi`) nu mai are `SourceInstaller[]`/`defaultInstallers`/bucla dinamica si nici context progresiv — compune **explicit**: `createSourceRuntime(deps)` -> `attachHttpClient.buildFrom(runtime)` -> `createSteamSource({...})` -> `createUpdates({...})` -> `createDeals({...})`, fiecare fabrica primind un obiect literal cu dependintele enumerate, iar registrul returnat e un literal `Object.freeze`-uit cu tip de retur inchis. Boot-ul (`main.ts`/`bootstrap.ts`) si `commandRuntimeContext` folosesc importuri ESM statice (migrarea ESM completa — Faza A: sintaxa; Faza B: emit ESM real cu `module: "NodeNext"` + `type: "module"` + extensii `.js` pe importurile relative; require-urile lenese ramase merg prin `createRequire(import.meta.url)` si functioneaza pe Node >=22.12 prin require(esm); singura exceptie e require-ul multi-linie tipat al lui `mongoContext` din bootstrap, cu `.default as typeof import(...)["default"]`), deci `satisfies AppRuntimeDeps` si return type-ul `CommandRuntimeContext` verifica wiring-ul real; completitudinea registrului de surse e dovedita la compilare de tipul de retur inchis, deci `requireSourceValue` si `assertNoUndefinedExports` (garzile de runtime pe chei) nu mai exista. La fel, `mongoContext` compune acum prin **factory-return imutabil, complet ca `sourceRegistry`**: fiecare modul de strat (cele 4 din `shared/` + cele 9 din `infra/mongo/`) expune un `buildFrom(context)` care **intoarce** contributia, iar `attachX(target)` deleaga la el (`Object.assign(target, buildXFrom(target))`). `createMongoContext` porneste de la o **copie proaspata** (`{ ...baseContext }`) si compune prin **spread in obiecte noi** (`{ ...prev, ...attachX.buildFrom(prev) }`) in ordinea dependentelor (`logging -> ... -> fetchSnapshots`), fara mutatie pe un context partajat in timpul compunerii; exportul singleton e `Object.freeze`-uit. Ordinea garanteaza ca niciun `buildFrom` nu citeste un camp adaugat ulterior (fara forward-reference). Modulele Mongo/shared isi **pastreaza** adaptorul `attachX(target)` ca thin-wrapper fiindca e folosit si direct de scripturi/teste de integrare (`acquireDbLock`, `guildSettingsCache`); exceptie `models.ts`, trecut complet pe factory-only (`export = { buildFrom }`, R5 #3) — consumatorii lui directi (`check-db-indexes`, `outboxLoadBenchmark`, testele de integrare outbox/seen) apeleaza `buildFrom` si isi fac singuri `Object.assign` pe contextul propriu. Ce a disparut nu e adaptorul, ci **punga** de pe input-ul lui: target-ul fiecarui adaptor e o interfata declarata, nu `Deps & Record<string, unknown>`.
...
- **Pachetele apt native se cache-uiesc ca arhive `.deb`.** Cele 14 pachete (cmake, clang, libclang-dev si headerele de compresie) insumeaza ~48 MB, iar mirror-ul Azure e inegal: masurat pe acelasi job, instalarea a durat 23s cand a mers bine, dar `cmake` a luat 89s si `libclang-dev` a blocat 3,5 minute intr-o rulare care a **expirat la timeout-ul de 6 minute si a picat CI-ul** — nu din cauza codului. Arhivele descarcate se pastreaza intr-un cache cu cheia derivata din `native-apt-packages.txt`, iar la hit `apt-get install` le gaseste in `/var/cache/apt/archives` si nu mai descarca nimic. Lista de pachete traieste in acel fisier, nu inline in fiecare workflow, ca `ci.yml` si `native-sanitizers.yml` sa nu divearga de cheia de cache. Salvarea foloseste `actions/cache/restore` plus un `actions/cache/save` conditionat pe main — `actions/cache` salveaza automat la finalul jobului, deci pe un PR fiecare branch si-ar fi scris propria copie, exact scurgerea de mai jos. Timeout-ul a crescut la 10 minute ca plasa pentru cazul in care cache-ul lipseste si mirror-ul e lent. Doua capcane, amandoua platite in CI si acum gardate: (1) **lista se citeste cu o cale ancorata la radacina repo** (`"$GITHUB_WORKSPACE/.github/workflows/native-apt-packages.txt"`), fiindca joburile au `defaults.run.working-directory: src` — `hashFiles()` si `path:` se rezolva din radacina, dar comenzile de shell mostenesc directorul jobului, deci forma relativa a picat cu `No such file or directory`, verificat de `workflowRootPathAnchoring.test.ts` pentru orice director care exista doar in radacina; (2) **instalarea trece `-o APT::Keep-Downloaded-Packages=true`**, fiindca apt sterge implicit arhivele dupa instalare — fara optiune, pasul care copiaza in `~/apt-archives` ar fi salvat un cache gol si fiecare rulare ar fi descarcat din nou tot, fara niciun semnal ca ceva e in neregula; pasul afiseaza acum si cate arhive au ramas, iar `cacheQuotaPolicy` cere optiunea in orice workflow care cache-uieste pachete apt.

- **Cache-urile de CI scriu doar de pe `main`, ca sa nu manance cota repo-ului.** Cota Actions e 10 GB, iar peste ea GitHub evacueaza LRU — adica exact layerele Docker si `target/`-ul cargo de care are nevoie rularea urmatoare. Doua scurgeri au dus repo-ul la 11,4 GB si au facut build-ul de container sa dureze 6 minute in loc de ~1: `docker/build-push-action` cu `cache-to: type=gha,mode=max` salveaza fiecare layer intermediar ca intrare separata, iar **fiecare PR isi scria propriul scope** (`refs/pull/N/merge`), o copie integrala a aceluiasi set (678 de intrari, 6,8 GB); iar `Swatinem/rust-cache` salva de pe orice branch o intrare de ~400 MB (14 copii, 4,2 GB). Acum `cache-to` din `container-scan.yml` e activ doar pe evenimente non-PR, iar `rust-cache` are `save-if: main`. **Citirea ramane deschisa tuturor branch-urilor**, iar `main` reimprospateaza intrarile: `ci.yml` la fiecare push pe main, `container-scan.yml` la fiecare push care atinge `Dockerfile`, lock-ul de dependinte sau `src/native/**` — adica fix intrarile care invalideaza cache-ul. Complementar, `cache-cleanup.yml` sterge intrarile unui PR cand acesta se inchide, ca sa nu se mai adune (masurat inainte de fix: 624 de intrari ramase de la PR-uri deja inchise = 5,3 GB, jumatate din cota). **Conditia de `cache-to` se scrie cu negatia pe prima pozitie.** Forma `este_PR && '' || valoare` pare sa opreasca exportul din PR-uri, dar in expresiile GitHub sirul gol e falsy, deci `||` cade mereu pe ramura de export: masurat, PR-urile scriau in continuare cache (3,03 GB, 31% din cota, si build-uri de 7-8 minute in loc de ~1m45s). Gate-ul `cacheQuotaPolicy` **evalueaza** acum expresia in loc sa caute un sir in ea — cele doua forme contin aceeasi comparatie si se comporta invers, deci o verificare textuala nu le distinge. **Curatarea la inchidere mai pierde o cursa**: evenimentul `closed` porneste imediat, dar o rulare de `container-scan` inca in derulare exporta cache dupa ce stergerea a terminat (masurat pe PR #971: inchis la 17:35:08, 24 de intrari buildkit aparute la 17:36:22). Trecerea saptamanala `sweep` din acelasi workflow sterge ramasitele PR-urilor deja inchise, indiferent cine a castigat cursa.
- Codul runtime nu mai foloseste abrevierea legacy pentru context; modulele de compatibilitate folosesc `target` pentru atasare si `deps` pentru factory-uri. Migrarea factory-urilor este incheiata: logica modulelor este expusa prin `createX(deps: XDeps): XApi` cu dependinte explicite, iar acolo unde mai exista, `attachX(target)` e doar adaptor subtire (`Object.assign(target, createX(...))`) cu target tipat printr-o interfata declarata, nu printr-o punga de chei. Tiparul factory este aplicat la `sources/steam`, `sources/deals`, `sources/updates`, `command-presentation` si `infra/http/client.ts`; **trei module au trecut complet pe factory-only, fara nicio forma de atasare pe target** (review R5 #3): `features/notifications/index` (exporta `{ createNotificationRuntime, createIsStillSubscribed, outboxSubscriptionFilter }`), `features/command-cache/commandCache` (exporta `{ createCommandCache, computeMissingChannelPerms, formatMissingChannelPerms }`) si `infra/mongo/models` (exporta `{ buildFrom }`); consumatorii lor (commandRegistry, mongoContext, testele e2e, `check-db-indexes`, testele de integrare Mongo) compun explicit prin factory/`buildFrom`, gardat de `commandInstallerTargetContracts.test.ts`; contractele de boot din `appRuntime` folosesc tipuri explicite (`CommandRuntime`, `ScraperRuntime`, `ActiveLocks`), iar factory-urile centrale din boot wiring (`createCronController`, `createOutboxWorker`, `createHttpServer`, `createHousekeeping`, `registerDiscordEvents`/`registerMongoEvents`, `createShutdownController`) primesc tipurile reale de deps exportate de modulele lor, cu `env` complet `RuntimeEnv` (gard compile-time in `appRuntimeTypedDeps.test.ts`). `SourceRegistryApi` si `MongoRuntimeContext` sunt value-tipate, iar `sources/sourceApis.ts` expune tipurile reale ale API-urilor de surse. Coalescing-ul inflight (`inflightAllGames`, `inflightDeals`, `activeEnrichments`) traieste in closure-ul fiecarei instante de factory, deci instantele cu deps diferite nu impart promisiuni. La nivel de modul raman doar cache-uri pure si deterministe (`enrichedCache`, cache-ul de regex). Singurele `[key: string]: unknown` ramase sunt intentionate si tin de **date**, nu de wiring: tipuri de payload dinamic, schema Mongo si documentele `lean()` (`GuildSettings & Record<string, unknown>` in `infra/mongo/guildSettings.ts`, unde campurile vechi trebuie sa treaca prin cod fara sa fie declarate). Niciun target de installer nu mai e `Deps & Record<string, unknown>`: adaptoarele ramase primesc interfete declarate. Ambele registre (`commandRegistry`, `sourceRegistry`) intorc contracte inchise compuse explicit (`RequiredCommandRegistry`, `SourceRegistryApi`), nu bag-uri de wiring.
- Testele din `src/test` nu mai folosesc abrevieri legacy de context sau tipuri wildcard nesigure; mock-urile Discord/Mongo/HTTP folosesc shape-uri locale si `unknown` pentru cazuri intentionat invalide. Testele sunt organizate pe domenii (review impact-mare #12): 10 subfoldere (`commands/`, `app/`, `sources/`, `notifications/`, `outbox/`, `gates/`, `mongo/`, `youtube/`, `security/`, `native/`), kiturile partajate (`*TestKit.ts`) si `fixtures/` la radacina, iar putinele teste cross-cutting raman flat; scripturile npm folosesc globuri recursive in ghilimele (`"dist/test/**/*.test.js"`).
- **Barrel-ul `types.ts` nu are footprint de runtime, dar ascundea incalcari de strat** (review nou, P2 #12). Premisa initiala ("~92 de importatori de runtime") nu mai era adevarata: barrel-ul compileaza in `export {};` (11 octeti), **zero** module importa valori din el si niciun modul emis nu-l incarca. Din cei 112 importatori type-only, 103 luau doar primitive. Problema reala erau ceilalti: opt module de productie trageau **contracte de domeniu** prin barrel (`DealInfo`, `GuildSettings`, `BotMetrics`, ...), ceea ce le ascundea dependinta adevarata. Repointate spre proprietar, `check-layer-imports` a picat imediat pe sase incalcari **preexistente** — deci barrel-ul functiona ca spalatorie pentru dependinte ilegale. Doua s-au rezolvat curat (`infra/http/httpMetrics.ts` si `sources/updates/updatesContracts.ts` foloseau doar 4, respectiv 2 campuri din `BotMetrics`: isi declara acum feliuta, deci dependinta spre `app` dispare complet). Doua raman explicit in lista `INVERSIUNE_DE_DEPENDINTA_NECESARA` din gate: `domain/deals/filtersCore.ts` si `shared/utilities.ts` sunt straturi PURE care au nevoie de contracte din features/sources — iesirea reala e mutarea contractelor in `domain/` si inversarea dependintei, nu un import mai frumos. Plafoanele de importatori au scazut la 90 productie / 108 total, iar un gate nou (`nimeni nu mai trage contracte de domeniu prin barrel`) prinde ce plafonul nu putea prinde: un import care nu creste numarul, dar ascunde domeniul.
- **CI ruleaza doua joburi in paralel, cu un singur check agregat** (review nou, P2 #15). Jobul `gates` e rapid si **fara Rust**: checkout, Node, `npm ci`, `build:ts`, gate-uri structurale + lint. Jobul `check` ramane cel complet (Rust, MongoDB, teste, benchmark, e2e). `ci` depinde de ambele, are `if: always()` si iese cu 1 daca oricare a picat — fara `always()`, un dependinte esuat ar face agregatorul `skipped`, iar protectia de branch ar vedea o stare care nu e nici succes, nici esec. Impartirea NU e o taiere in doua a jobului existent, si masuratoarea explica de ce: pe un run reusit, pasii scumpi sunt restaurarea cache-ului Rust (96s) si `apt-get` pentru librariile C (34s), nu verificarile in sine (37s `check` + 28s clippy). Doua joburi care ar avea amandoua nevoie de Rust ar plati de doua ori acele 130s ca sa castige cateva zeci de secunde. Jobul `gates` castiga tocmai fiindca nu are nevoie de ele — verificat ca gate-urile trec cu addon-ul nativ mutat din arbore. Gardat de `ciJobTopology.test.ts`.
- **Fiecare job isi declara `timeout-minutes`, iar instalarea de pachete nu mai poate astepta la infinit.** Un `apt-get` a ramas blocat 12 minute pe pasul de librarii C (normal ~15s): asteapta implicit la nesfarsit dupa lock-ul dpkg tinut de `unattended-upgrades`, iar fara timeout de job limita reala era cea implicita de 6 ore — PR-ul ramanea agatat in loc sa pice si sa poata fi reluat. Toate joburile din cele 12 workflow-uri au acum un plafon (5-45 de minute, dupa durata masurata), iar pasul de apt are timeout propriu de 6 minute, opreste `apt-daily`/`unattended-upgrades`, foloseste `-o DPkg::Lock::Timeout=120` si reia de trei ori. Gardat de `workflowJobTimeouts.test.ts`. Tot acolo se verifica si ca niciun workflow declansat de `pull_request` nu filtreaza dupa branch-ul tinta: `branches: [main]` sub `pull_request:` insemna ca un PR stivuit peste alt PR primea 3 check-uri in loc de 9, fara niciun semn ca lipseste ceva.
...
src/
  ...
  features/
    ...
    notifications/
      ...
      notificationBatchExecutor.ts
      notificationCycleRepository.ts
      outboxTypes.ts
      outboundChannel.ts
      seenRepository.ts
...
- `setInteractionHandler.ts` - subcomenzile directe ramase din `/set`; construirea planului de update per subcomanda e delegata modulului pur `setUpdatePlan.ts`;
...
- `outboxAdminHandler.ts` - adaptor operational legacy pastrat intern, dar neatasat definitiilor slash si registrului de comenzi;
...
- `statusInteractionHandler.ts` - `/status game` si `/status watchlist`, cu stari explicite, izolare per sursa si paginare;
...
- `templateAndNotificationPreviewHandler.ts` - administrarea unificata a template-urilor si preview fara efecte secundare;
- `watchlistCoverageAndAliasHandler.ts` - acoperirea surselor din watchlist si aliasurile locale serverului;
...
- Aprobarea bot-add este one-time si atomica, legata de botul si solicitantul exact identificat din audit log. Un bot fara aprobare valida este eliminat si incidentul este auditat.
- Inspectia de continut separa explicit `safe`, `uncertain` si `confirmed`: numai continutul confirmat este sters, iar alertele nu reproduc payload-ul analizat.
- Delegarea permisiunilor Discord sensibile prin modificarea rolurilor sau atribuirea lor membrilor este monitorizata prin audit log; modificarile facute de altcineva decat owner sunt restaurate la starea anterioara.
- Moderarea trateaza Discord si Mongo ca doua sisteme cu rollback si reconciliere explicite, curata starea expirata si nu persista motivele sensibile ale avertismentelor.
...
### Gate-uri structurale: proprietate pe AST, nu potrivire de text

Regulile de arhitectura se verificau pana acum citind fisierul sursa ca text si potrivind sabloane: `assert.match(text, /function createAppServices/)`, `text.includes("Object.assign(base")`, `new RegExp("^export interface " + name)`. Problema nu e ca metoda nu prinde nimic, ci **ce** prinde: sirul de caractere, nu proprietatea. Un gate scris asa pica la reformatare, la mutarea unei linii, la redenumirea unei variabile locale sau la spargerea unui apel pe doua randuri, si trece daca acelasi sablon apare intr-un string sau intr-un nume de test. Invers, cand codul se schimba corect, gate-ul cere sa fie rescris — ceea ce inseamna ca **el pinuia forma criticata**, nu regula.

`src/test/gates/sourceStructureQueries.ts` e biblioteca de interogari care rezolva asta: parseaza modulul cu compilatorul TypeScript (`ts.createSourceFile`) si expune interogari peste noduri — `functions`/`findFunction` (nume, `async`, parametri cu tip/optional/valoare implicita, tip de retur), `membersOf`/`allMembers`/`nestedMembers` (membri de contract, inclusiv din intersectii si literale imbricate), `imports` (modul, `type`-only, alias real vs. legat), `requireSpecifiers`, `assertions` (`as never`, dubla asertiune), `calls`/`callsWithin`/`callBoundLocals`, `compositionLayers` (straturile de spread ale unei compuneri), `mutatedPropertyPaths` (scrieri `=`/`+=`/`++` pe un camp), `exportedTypeNames`/`exportedFunctionNames`, `namedObjectProperties`, `defaultExportName`, `topLevelFrozenExports`. Este singurul modul care mai citeste sursa ca text, si o face pe un singur rand, chiar inainte de `createSourceFile`.

Gate-uri migrate, in ordinea in care au fost mutate: `registryClosedContracts.test.ts` (cel mai afectat — avea peste 100 de asertari textuale, inclusiv linii de compunere din `mongoContext` fixate caracter cu caracter), `metricRecorderBoundaries.test.ts`, `commandInstallerTargetContracts.test.ts`, `mongoPorts.test.ts`, `roleCompositionRoots.test.ts`, `guildDocumentSplit.test.ts`, `securityHandlerLayering.test.ts`, `portsHaveProductionConsumers.test.ts`, `guildSettingsBusOwnership.test.ts`, `notificationCycleReuse.test.ts`, `metricsOnlyThroughRecorders.test.ts`, `runConcurrentSingleContract.test.ts`, `mongoSchemaDomains.test.ts`, `gameCatalogSeparation.test.ts`, `notificationKindRegistry.test.ts`, `sourceTypeValidatorsLocality.test.ts`. Lista din text nu se poate desincroniza de cod: `docsClaimsMatchCode.test.ts` compara enumerarea de aici cu `MIGRATED_GATES` din gate-ul care pazeste migrarea. Diferenta se vede la compunerea `mongoContext`: in loc de linia exacta `attachModelsModule.buildFrom(withUtilities)`, gate-ul verifica acum **proprietatea** — fiecare strat are exact un `buildFrom`, extinde strict stratul anterior si primeste ca argument un strat deja construit, niciodata unul viitor. Un strat nou trece fara sa atinga testul; o compunere care sare peste un strat sau citeste in avans pica.

`structuralGatesUseAst.test.ts` pazeste migrarea: gate-urile declarate migrate nu au voie sa mai foloseasca `readFileSync`, `assert.match` sau `new RegExp`; biblioteca de interogari nu are voie sa contina asertari; un test de reformatare demonstreaza ca acelasi cod scris dens si scris spatiat da exact aceleasi straturi de compunere; iar numarul de gate-uri care inca citesc text are un plafon care poate doar sa scada (13 in prezent). Gate-urile functionale — cele care ruleaza cod si verifica comportament — rezolva o alta problema si rezolva bine; ele rămân cum sunt.

## Zone ramase de curatat

Lista de mai jos e scrisa ca sa fie verificabila, nu incurajatoare: fiecare punct spune ce s-a facut SI ce a ramas masurabil.

- **Documentul `Guild` e inca un god-object de persistenta.** Doua domenii au iesit in colectii dedicate — moderarea (`guildModeration`, cu backfill lenes la citire) si securitatea (`guildSecurity`, cu oglindire pe scriere). Au ramas pe documentul `Guild`: configurarea generala, notificarile pe sase canale (update-uri, reduceri, DLC, YouTube, player-count, future-release) si inregistrarile admin. Motorul de tranzitie e comun (`shared/guildDomainSliceStore.ts`), deci urmatorul domeniu inseamna o lista de campuri plus o colectie, nu logica noua.
- **Migrarea securitatii e pe jumatate, prin proiectare.** Colectia dedicata primeste scrierile, dar sursa de citire ramane `Guild` fiindca acele cai citesc prin cache-ul de setari, nu prin model. Pasul care lipseste e mutarea citirilor pe colectia dedicata, cu un backfill explicit; pana atunci colectia noua e o replica, nu o sursa.
- **Gate-urile structurale sunt migrate pe AST doar partial.** Sapte gate-uri interogheaza AST-ul; restul citesc inca textul sursei, cu un plafon descrescator (25) impus de `structuralGatesUseAst.test.ts`.
- **Incarcarea modulelor in teste nu e integral ESM.** 28 de fisiere din `src/test` folosesc `createRequire`, cu plafon descrescator. Codul de productie nu are exceptii.
- **Gate-urile structurale sunt migrate pe AST doar partial.** Gate-urile structurale migrate interogheaza AST-ul; restul citesc inca textul sursei, cu un plafon descrescator (17) impus de `structuralGatesUseAst.test.ts`.
- **Incarcarea modulelor in teste nu e integral ESM.** 43 de fisiere din `src/test` folosesc `createRequire`, cu plafon descrescator. Codul de productie nu are exceptii.
- **Routerele de comenzi nu sunt toate impartite.** Routerul de securitate a fost spart in use-case-uri, prezentare si adaptoare (461 -> 251 de linii, zero `try`/`catch`). Handler-ul de moderare (343 de linii) urmeaza acelasi tipar si nu a fost inca refacut.
- Contextul comun din runtime si registry a fost restrans: factory-urile si handler-ele primesc `deps` explicit, iar `commandRuntimeContext` este o factory tipata consumata de `commandRegistry` cu context proaspat per apel. Daca apar module noi de comenzi, ele trebuie sa pastreze acelasi model.
- Mentinerea testelor fara tipuri wildcard nesigure sau abrevieri legacy de context cand se adauga mock-uri noi.
- Mentinerea documentatiei sincronizate la fiecare schimbare de cod — cifrele din acest fisier sunt verificate automat de `docsClaimsMatchCode.test.ts`, ca sa nu ramana in urma tacut.
