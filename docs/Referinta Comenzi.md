...
| `/stop anti-raid` | Admin, Ephemeral | Opreste modulul anti-raid; owner-only, cu confirmare obligatorie si doar cand exista un incident intr-o etapa de interventie. Nota: Este alta operatiune decat /anti-raid force-stop, care doar incheie un incident in curs. Nota: Etapele care permit oprirea sunt confirmed, containment, cleanup si recovery; fara raid tratat comanda refuza si spune de ce. Nota: Dupa oprire serverul ramane fara detectie de raid. | `/stop anti-raid confirm:true` |
