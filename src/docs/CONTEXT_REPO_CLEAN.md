...
- Comentariile explicative din fisierele de cod au fost eliminate. Daca un rationale trebuie pastrat, el trebuie pus in documentatia potrivita dupa subiect, nu langa implementare. Singura exceptie acceptata sunt notele scurte de o linie la punctele de concurenta/race condition (de exemplu ordinea „invalidare token inainte de eliberarea lock-ului" din `cron.ts`), unde ordinea operatiilor este subtila si comentariul previne reintroducerea bug-ului; nota ramane scurta si nu inlocuieste documentatia. Regula este aplicata automat de `scripts/check-no-comments.ts` (parte din `npm run check`, deci si in CI): scaneaza `.ts`/`.js`/`.rs` (parser TypeScript pentru TS/JS ca sa nu existe fals pozitive pe regex/URL; scanner cu ignorare de string-uri pentru Rust) si esueaza la orice comentariu, cu exceptia unui allowlist explicit din script (cele doua note din `cron.ts`).
...
- `outboxAdminHandler.ts` - comenzile admin `/outbox` (`status`, `deadletters`, `retry`, `pause`, `resume`, `recovery-verify status`) pentru operarea outbox-ului (coada per-guild si globala, dead-letter, reprogramare livrari, pauza/reluare drenare, stare recovery-verify); protejat de admin guard (`outbox` e in lista de comenzi admin). `pause`/`resume` comuta flagul persistent `outboxPaused` (pe `system_state`, via `getOutboxPaused`/`setOutboxPaused`), pe care worker-ul de drenare il verifica la fiecare tick inainte de a lua lock-ul;
