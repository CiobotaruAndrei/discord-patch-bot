...
- Codul runtime nu mai foloseste abrevierea legacy pentru context; modulele de compatibilitate folosesc `target` pentru atasare si `deps` pentru factory-uri. Migrare incrementala in curs: se reduce cuplajul prin `target`/`Object.assign` mutand logica fiecarui modul intr-un factory pur `createX(deps: XDeps): XApi` cu tipuri explicite (fara `[key: string]: unknown` si fara campuri output auto-referentiale in interfata de context), iar `attachX(target)` ramane doar un adaptor subtire la margine (`Object.assign(target, createX(...))`). Module migrate dupa acest tipar: `sources/steam` (`createSteamSource`, pasul 1, fara stare de modul), `sources/deals` (`createDeals`, pasul 2) si `sources/updates` (`createUpdates`, pasul 3). `deals` pastreaza intentionat starea de proces la nivel de modul (cache-urile `activeEnrichments`/`enrichedCache`/`inflightDeals` si un `deps` setat de `createDeals`), fiindca starea e legata de proces, nu de un consumator anume; smell-urile de tipare (context lax + campuri auto-referentiale) sunt insa eliminate ca la `steam`. `updates` foloseste acum acelasi tipar ca `steam`/`deals`: `attachUpdates(target)` construieste un obiect `deps` explicit tipat (`UpdatesDeps`) din campurile numite ale lui `target` si il paseaza catre `createUpdates`, deci `deps` este un snapshot, NU o referinta vie la target — nicio mutatie tarzie a contextului nu se mai scurge in factory. Singura dependinta cu adevarat injectabila dintre fostele campuri output este `executeFetchWithCircuitBreaker` (override folosit ca seam de test in `getLatestForAllGames`), pastrat ca dependinta optionala explicit tipata in `UpdatesDeps`; seam-ul de test injecteaza acum acest override prin `deps` la momentul attach (up-front), nu prin mutarea contextului partajat dupa attach. Acoperit de un test de regresie in `perSourceConcurrency.test.ts` (o mutatie tarzie a contextului nu modifica rezultatul). `features/notifications/index` are factory-ul `createNotificationRuntime` cu semnatura stransa la un tip precis `NotificationsRuntimeDeps` (fara `& Record<string, unknown>`); stramtorarea a scos la iveala trei dependinte care se scurgeau anterior prin index signature-ul lax (`saveFetchSnapshot`, `NotificationOutboxModel`, `NotificationOutboxSentModel`), acum declarate explicit tipate. `NotificationsContext = NotificationsRuntimeDeps & Record<string, unknown>` ramane doar pentru adaptorul de atasare. Pasul 4 a curatat factory-urile de comenzi care existau deja (`command-cache` `createCommandCache`, `command-presentation` `createCommandPresentation`): contractul de input al factory-ului a fost spart in `CommandCacheDeps` / `CommandUiDeps` (strict, fara `[key: string]: unknown`), iar `XContext = XDeps & Record<string, unknown>` ramane doar pentru adaptorul de atasare (tinta dinamica). Bag-ul central de wiring `CommandRegistryContext` (din `commandRegistry.ts`) ramane intentionat dinamic cu `[key: string]: unknown` — este echivalentul lui `Record<string, unknown>` folosit de `sourceRegistry`, pe care installerii il populeaza cu chei arbitrare, deci nu este un contract de input de factory. Factory-ul fiecarui modul e expus si ca proprietate pe adaptor (`attachX.createX`) pentru testare cu deps explicit tipate. Pasul 5 (finalizare) a curatat `infra/http/client.ts`: `HttpClientContext` a fost spart in `HttpClientDeps` (intrari numite strict) + `HttpClientContext = HttpClientDeps & Record<string, unknown>` pentru `attachHttpClient` (care ramane un singur adaptor, nu factory separat, din cauza slot-ului partajat mutabil `metricsRef`). Dupa acest pas, migrarea este completa: singurele `[key: string]: unknown` ramase sunt intentionate — tipurile de date dinamice (`types.ts`), schema Mongo (`infra/mongo/models.ts`) si bag-ul de wiring `CommandRegistryContext` — niciunul nu este un contract de input de factory. La nivel de compozitie, `appRuntime` (peste care `main.ts` e doar un entry point subtire) injecteaza dependintele prin contracte explicit tipate: `CommandRuntime` (metodele registry-ului de comenzi folosite efectiv), `ScraperRuntime` (`attachMetrics`/`cleanEnrichedCache`/`getEnrichedCacheSize`) si un context Mongo cu `env: AppEnv` + `activeLocks: ActiveLocks` — fara `Record<string, AnyFn>` sau `& Record<string, unknown>` pe suprafata de injectie de boot. Pasul 6 a strans contractul de **citire** al celor doua bag-uri de registru: `MongoRuntimeContext` (din `mongoContext.ts`) si `SourceContext` (din `sourceRegistry.ts`) au fost schimbate din `Record<string, unknown>` in `Record<MongoContextExportKey, unknown>` / `Record<SourceRegistryExportKey, unknown>`. Astfel, citirile din `buildMongoContextExports` / `buildSourceRegistry` (~75 de accesari de chei) sunt acum **typo-safe la compilare** — un acces gresit de cheie (ex. `context.GuildModle`) e eroare `tsc`, nu `unknown` tacut prins abia la boot de `assertNoUndefinedExports`. Installerii nu sunt afectati (sunt `require`-uiti si cast-uiti la `XInstaller`, deci isi pastreaza propriile tipuri de parametru). Bag-ul de wiring `CommandRegistryContext` ramane intentionat dinamic (chei arbitrare de la installerii de comenzi). Acoperit de `typedWiringContext.test.ts` (ambele registre expun toate cheile declarate, definite). Pasul 7 a dus contractul lui `sourceRegistry.ts` mai departe de la chei typo-safe la **valori tipate**: `SourceContext` si tipul de retur al lui `buildSourceRegistry` sunt acum tipul-alias `SourceRegistryApi` (un alias de obiect, NU `interface` — un alias ramane assignable la `Record<string, unknown>`, cerinta semnaturii lui `assertNoUndefinedExports`), in care fiecare dintre cele 30 de chei are o semnatura concreta in loc de `unknown`. Astfel cele 4 exporturi named ale registrului (`dealHash: (deal: unknown) => string`, `safeCheerioLoad: (html: unknown) => CheerioAPI`, `extractOfferEndFromHtml: (html: unknown) => string | null`, `MAX_HTML_BYTES: number`) au tipuri precise pentru consumatorii care le importa tipat, nu `unknown`. Constrangere documentata: modulele de sursa (`steam`/`deals`/`updates`) folosesc `export =` (adaptorul installer), care **interzice** exporturi named de tip (TS2309), deci interfetele lor `SteamSourceApi`/`DealsApi`/`UpdatesApi` si tipurile-domeniu (`DealInfo`, `SteamSearchItem` etc.) nu pot fi importate aici fara a le muta in fisiere de tip partajate; din acest motiv intrarile/iesirile de domeniu ale functiilor de sursa raman `unknown` (o tipare integrala a lor e un pas viitor, separat, care ar converti `export =` la exporturi named). Schimbarea e pur interna (toti consumatorii folosesc `require`, deci zero cascada). Acoperit de `sourceRegistryTypedApi.test.ts` (aserțiuni compile-time `Expect<...>` ca cele 4 exporturi named nu sunt `unknown` + verificari de shape la runtime pentru toate cele 30 de chei). Acelasi Pasul 7 a fost aplicat si lui `mongoContext.ts` (46 de chei): `MongoRuntimeContext` si returul lui `buildMongoContextExports` devin un alias de obiect cu semnaturi concrete — castigul vizibil e ca cele 13 modele sunt tipate `Model<any>` (tipul vine din `mongoose`, importabil), `activeLocks` e `Map<string, unknown>`, `SchemaDriftError` un `new (...args) => Error`, iar functiile au parametri reali (39/46 chei tipate concret); tipurile-domeniu raman `unknown` din acelasi motiv (`export =` pe modulele installer). Acoperit si de `mongoContextTypedApi.test.ts`. Pasul 8 a inchis ambele datorii ramase din Pasul 7 pentru surse: (1) interfetele `SteamSourceApi`/`DealsApi`/`UpdatesApi` au fost mutate in modulul partajat de tipuri `sources/sourceApis.ts` (modul ES normal, importabil cu `import type` inclusiv din modulele `export =`), deci `SourceRegistryApi` e compus acum din **tipurile reale** (indexed access pe cele 3 API-uri + `DealInfo`/`NormalizedUpdate`/`PatchUpdate` din `types.ts`) si e exportat; (2) `createUpdates`/`createDeals` nu mai seteaza un `deps` global de modul — toate functiile dependente de deps traiesc in closure-ul factory-ului, deci doua instante cu deps diferite nu se mai suprascriu (regresie acoperita in testele functionale ale ambelor surse). Hartile de coalescing inflight (`inflightAllGames`, `inflightDeals`, `activeEnrichments`) traiesc in closure-ul factory-ului: o promisiune inflight e legata de deps-ul instantei care a pornit-o, deci doua instante nu isi pot imparti rezultate produse cu dependinte diferite (cheia `mode:hash(chei)` nu mai are nevoie de fingerprint de config). La nivel de modul raman doar cache-urile pure de date, cu continut determinist per cheie: `enrichedCache` (deal imbogatit per `id:currency`) si cache-ul de regex per `GameConfig`.
