...
- `adminAccessCodeHandler.ts` - `/admin-access unlock|status|revoke`, guild-only si ephemeral, pentru granturi admin temporare activate cu cod din `BOT_ADMIN_ACCESS_CODES`;
- `adminCommandAccessHandler.ts` - `/set admin-command-access`, `/admin-command-access list` si `/delete admin-command-access`, owner-only, pentru rol exact sau rol egal/mai mare care poate folosi comenzi admin pe langa `Administrator` si granturile temporare prin cod;
