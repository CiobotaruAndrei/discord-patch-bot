...
- `createSourceRegistry(): SourceRegistryApi` (fara parametri) compune **explicit**: construieste un **context proaspat per registry** (`freshSourceContext()` = copie shallow a modulului `runtime`), apoi apeleaza `attachHttpClient(context)` -> `attachSteam(context)` -> `attachUpdates(context)` -> `attachDeals(context)` in ordinea dependentelor (nu mai exista lista dinamica `defaultInstallers: SourceInstaller[]` + bucla). Cele patru `attach*` muta copia proaspata, nu singletonul `runtime` partajat; exportul singleton `registry` ramane un wrapper peste un astfel de build, iar `buildSourceRegistry`/`requireSourceValue` extrag exportul **inchis** `SourceRegistryApi` (fail-fast pe cheie lipsa). Garda in `sourceRegistry.functional.test.ts` (registry-ul are toate exporturile + modulul `runtime` nu capata chei dupa un build) si `registryClosedContracts.test.ts` (fara `SourceInstaller`/`defaultInstallers`, apeluri `attach*` ordonate).
