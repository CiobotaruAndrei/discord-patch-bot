...
- Installerele au tipul `(context: never) => void` si NU sunt coercitate per-modul (`as unknown as` interzis prin gard); singura granita nesigura e `install(context as never)` in bucla de instalare, pinuita prin gard la exact o aparitie per registru si interzisa in restul codului. **De ce nu un "context real" in loc de `never` (decizie verificata, review #10.1):** tipizarea compunerii cere ca registrul sa satisfaca simultan TOATE contextele declarate local de module — incercarea reproduce 19 erori tsc (chei produse de installers anteriori: `safeDefer`/`COLORS`/`checkUserCooldown`/`SEEN_PER_GAME_LIMIT` etc.; conflicte structurale `logger`/`handleInteraction`; variancea builder-elor "Like" vs clasele discord.js). Mai fundamental: aceleasi chei (ex. `safeDefer`) sunt declarate de fiecare handler cu propriul tip structural ingust (interaction shape diferit per handler), iar supratipul comun al unor vederi independente colapseaza in `never`/`any` — adica exact bypass-ul evitat. `never` pe parametru e encoding-ul corect al compunerii existentiale; alternativa reala ramane DI per handler (factory-uri), directia documentata mai jos.
...
- Lantul de drain e tipat cu `OutboxDiscordClient` (= `NotificationDiscordClient & { isReady() }`): `outboxDelivery` (client ne-ready = esec tranzitoriu, nu dead-letter), `drainOutbox` din runtime, registry, `appRuntime`, `outboxWorker` si `/outbox drain-now` — fara `client: unknown` nici pe drain.
