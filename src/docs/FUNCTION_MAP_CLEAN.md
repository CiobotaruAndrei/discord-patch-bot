...
- Installerele legacy nu mai sunt apelate prin `install(context as never)`. Lista de installers ramane progresiva si dinamica, dar bucla accepta un `LegacyInstallerTarget = Record<string, unknown>`, verifica runtime ca fiecare element este functie (`isCommandModuleInstaller`) si pastreaza validarea fail-fast a iesirilor prin `requireRegistryFunction`. **De ce nu un "context real" complet aici:** tipizarea compunerii cere ca registrul sa satisfaca simultan TOATE contextele declarate local de module; incercarea reproduce erori tsc pe chei produse de installers anteriori (`safeDefer`/`COLORS`/`checkUserCooldown`/`SEEN_PER_GAME_LIMIT` etc.) si conflicte structurale `logger`/`handleInteraction`. Fix-ul curent elimina `never` si pastreaza granita legacy explicita; alternativa finala ramane DI per handler, fara registru progresiv.
