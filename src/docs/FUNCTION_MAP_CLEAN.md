...
- Installerele nu mai sunt apelate prin `install(context as never)` si nu mai folosesc `LegacyInstallerTarget = Record<string, unknown>`. Boundary-ul de instalare nu mai are **niciun cast**: `installCommandModules` e non-generic si tinta e o atribuire tipata explicit (`const installContext: CommandInstallerTarget = context`), fiindca `CommandRuntimeBootContext` e assignable la contractul **all-optional** `CommandRegistryContext` (cheile pe care le adauga installer-ele sunt toate optionale, deci absenta lor initiala e validata de tipuri, nu fortata cu `as`). Lista de installers ramane progresiva si dinamica, verifica runtime ca fiecare element este functie (`isCommandModuleInstaller`) si pastreaza validarea fail-fast a iesirilor prin `requireRegistryFunction`. Garda din `registryClosedContracts.test.ts` pinuieste zero `context as` pe boundary. **De ce inca progresiv:** tipizarea compunerii cere ca registrul sa satisfaca simultan TOATE contextele declarate local de module; incercarea reproduce erori tsc pe chei produse de installers anteriori (`safeDefer`/`COLORS`/`checkUserCooldown`/`SEEN_PER_GAME_LIMIT` etc.) si conflicte structurale `logger`/`handleInteraction`. Alternativa finala (ca sa dispara si contextul comun mutat) ramane DI per handler, fara registru progresiv.
