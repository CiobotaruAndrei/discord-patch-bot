...
test("cpuBenchmark: runAreaBenchmarks acopera fuzzy/hashing/autocomplete/deal-filters cu paritate native==TS", () => {
  ...
  for (const expected of ["fuzzy-match (findGameKeys)", "hashing (dealHash)", "autocomplete (buildAutocompleteChoices)", "deal-filters (dealPassesFilters)", "listing-batch (extractAndRankListingCandidates)"]) {
