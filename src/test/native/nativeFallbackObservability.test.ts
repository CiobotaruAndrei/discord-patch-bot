...
test("NATIVE_FALLBACK_FUNCTIONS enumera doar functiile inca native-primary (cele care pot face fallback)", () => {
  for (const fn of ["classifyPatchNote", "scoreListingCandidate", "rankListingCandidates", "extractAndRankListingCandidates", "chooseBestSteamMatch", "isGoodSteamArticleUrl", "extractDateScore", "inspectUntrustedContent", "inspectMagic", "scanYara"]) {
    assert.ok(fuzzy.NATIVE_FALLBACK_FUNCTIONS.includes(fn), `lista canonica include ${fn}`);
  }
  assert.equal(fuzzy.NATIVE_FALLBACK_FUNCTIONS.length, 10);
