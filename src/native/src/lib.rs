...
fn score_listing_candidate_impl(href: &str, text: &str, keywords: &[String]) -> u32 {
...
pub fn score_listing_candidate(href: String, text: String, keywords: Vec<String>) -> u32 {
  score_listing_candidate_impl(&href, &text, &keywords)
}
  ...
  let scores: Vec<(u32, f64)> = candidates
    ...
    .map(|candidate| {
      let score = if has_keywords {
        score_listing_candidate_impl(&candidate.href, &candidate.text, &keywords)
      } else {
        0
      };
      (score, extract_date_score_impl(&candidate.href))
    })
    .collect();
