...
type RecoveryMetrics = { channelLockRecoveryRuns?: number; channelLockRecoveryFailures?: number; channelLockRecoveriesConverged?: number };
...
export function createChannelLockRecoveryTask(deps: CreateChannelLockRecoveryTaskDeps): ScheduledTaskRunner {
  ...
  return createScheduledTaskRunner({
    ...
    task: async () => {
      ...
      deps.metrics.channelLockRecoveriesConverged = (deps.metrics.channelLockRecoveriesConverged ?? 0) + totals.converged;
    },
    onResult: result => {
      if (result.status === "completed") {
        consecutiveFailures = 0;
        deps.metrics.channelLockRecoveryRuns = (deps.metrics.channelLockRecoveryRuns ?? 0) + 1;
        return;
      }
      if (result.status !== "failed") return;
      consecutiveFailures++;
      deps.metrics.channelLockRecoveryFailures = (deps.metrics.channelLockRecoveryFailures ?? 0) + 1;
