...
function createSchedulers(deps: AppRuntimeDeps, services: RuntimeServices): Schedulers {
  ...
  const { client, recorders, lifecycle, config, games, rateLimiter } = services;
  const cronController = createCronController({
    ...
    client, games, config, metrics: recorders.cron, lifecycle, errorMessage, errorDetail, requestContext
  });
  ...
  const outboxWorker = createOutboxWorker({
    ...
    lifecycle, metrics: recorders.outbox, errorMessage, adminAlert, isPaused: () => getOutboxPaused(),
