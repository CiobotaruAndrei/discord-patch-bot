...
export const CORE_COMMAND_ACCESS: readonly CommandAccessRule[] = [
  ...
  { command: "suggest-command", access: "public", discordAdminPermissions: false },
  { command: "list", access: "admin", discordAdminPermissions: true },
  { command: "add", access: "admin", discordAdminPermissions: false, publicSubcommands: ["suggestion", "watchlist-game"] },
...
export const CORE_CATALOG_HELP: readonly CommandCatalogHelpEntry[] = [
  ...
  { command: "/suggest-command", ephemeral: true, description: "Permite unui user sa propuna direct o comanda noua, cu numele si descrierea functionalitatii dorite.", example: "/suggest-command name:calendar description:Sa arate urmatoarele update-uri programate" },
  { command: "/list suggest-command", description: "Listeaza comenzile propuse de useri pe server, cu numele propus si ce ar trebui sa faca.", example: "/list suggest-command numar:10" },
  { command: "/add watchlist-game", ephemeral: true, description: "Propune un joc nou pentru lista botului; este aliasul clar al rutei /watchlist-game add.", example: "/add watchlist-game game:silksong" },
