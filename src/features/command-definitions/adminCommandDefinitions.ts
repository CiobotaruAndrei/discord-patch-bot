...
export function buildAdminCommandDefinitions({ SlashCommandBuilder, PermissionsBitField, CURRENCY_CHOICES }: SlashDefinitionTools): SlashCommandJsonSource[] {
  return [
    ...
    new SlashCommandBuilder()
      ...
      .addSubcommand(subcommand => subcommand.setName("update-template").setDescription("Sablon mesaj pentru notificarile de update (gol = implicit)")
        .addStringOption(option => option.setName("value").setDescription("Text; foloseste {count} pentru numarul de update-uri. Gol = reset la implicit").setRequired(false)))
      .addSubcommand(subcommand => subcommand.setName("discount-template").setDescription("Sablon mesaj pentru notificarile de reduceri (gol = implicit)")
        .addStringOption(option => option.setName("value").setDescription("Text; foloseste {count} pentru numarul de reduceri. Gol = reset la implicit").setRequired(false)))
