...
export const COMMAND_HELP_ENTRIES: readonly CommandHelpEntry[] = [
  ...
  { command: "/set admin-command-access", permissions: "Admin top-level, owner-only runtime, Ephemeral", description: "Seteaza rolul care poate folosi comenzile admin pe langa Administrator si codurile de acces temporare. Mode `role` accepta doar rolul ales, iar `role-or-higher` accepta rolul ales sau unul mai mare.", example: "/set admin-command-access role:@Moderator mode:role-or-higher", notes: ["Pana ownerul seteaza aceasta regula, rolurile nu dau acces admin; raman Administrator si granturile temporare activate prin cod."] },
  { command: "/admin-command-access list", permissions: "Admin top-level, owner-only runtime, Ephemeral", description: "Afiseaza regula curenta de acces admin prin rol pentru server.", example: "/admin-command-access list" },
  { command: "/delete admin-command-access", permissions: "Admin top-level, owner-only runtime, Ephemeral", description: "Sterge regula de rol pentru comenzile admin si revine la accesul implicit.", example: "/delete admin-command-access confirm:true" },
  { command: "/admin-access unlock", permissions: "Public, guild-only, Ephemeral", description: "Activeaza temporar pentru userul tau accesul la comenzile admin folosind codul configurat in secretul BOT_ADMIN_ACCESS_CODES.", example: "/admin-access unlock code:cod-primit-de-la-owner", notes: ["Grantul este per server, expira dupa 30 de minute si nu salveaza codul in Mongo."] },
  { command: "/admin-access status", permissions: "Public, guild-only, Ephemeral", description: "Afiseaza daca userul tau are un grant admin activ prin cod pe serverul curent.", example: "/admin-access status" },
  { command: "/admin-access revoke", permissions: "Public, guild-only, Ephemeral", description: "Revoca grantul admin prin cod pentru userul tau pe serverul curent.", example: "/admin-access revoke" },
