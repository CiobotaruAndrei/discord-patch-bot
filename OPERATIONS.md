...
| `guildAuditEntryClaims` | `{ guildId, entryId }` | `expiresAt` (10 minute) | revendicarea unei intrari din Audit Log deja corelate cu o resursa protejata (`_id` = `guildId:entryId`). Inainte era un `Set` in memorie golit la 500 de intrari: dupa golire sau dupa un restart, aceeasi intrare redevenea eligibila si putea fi atribuita a doua oara. Revendicarea e un `$setOnInsert`, deci prima corelare castiga; TTL-ul e mai lung decat fereastra de corelare, ca inregistrarea sa nu expire cat timp intrarea mai poate fi vazuta in Audit Log |
